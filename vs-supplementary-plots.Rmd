...
figure_out_labels <- function(col_name) { 
  if (is_match(col_name, 'HR')) { 
    ...
    # return(expression(paste('ln (EDA [', mu, 'S])'))) 
...
# get_eda_y_limits <- function(df) {
#   if (max(df$Value) < 10) {
#     c(y_min, 10)
#   } else if (min(df$Value) > 10) {
#     c(10, y_max)
#   } else {
#     c(5, 15)
#   }
...
draw_dummy_plot <- function(title) {
  dummy_plot <- ggplot(dummy_df, aes(x=x, y=y, group=group)) + 
    ...
    theme(
        ...
        axis.title.x=element_text(size=82),
...
draw_plots <- function() {
  ...
  # subj_list <- c('T037', 'T046')
  # subj_list <- c('T061', 'T063')
  ...
  for(subj in subj_list) {
    ...
    for (session in session_list) {
      ...
      title <- paste0(subj, ': ', get_session_name(session))
      ...
      if(nrow(hr_df)>0) {
        ...
        temp_plot_list[[length(temp_plot_list)+1]] <- draw_dummy_plot(title)
      }
      ...
      if(nrow(eda_df)>0) {
        eda_plot <- eda_df %>%
          ...
          #                    limits=get_eda_y_limits(eda_df),
          #                    breaks=get_eda_y_breaks(eda_df))
          # scale_y_continuous(trans='log10',
